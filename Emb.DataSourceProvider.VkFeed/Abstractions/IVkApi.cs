...
namespace Emb.DataSourceProvider.VkFeed.Abstractions
{
    ...
    {
        [Get("/method/newsfeed.get?return_banned=0&count=100&filters=post&access_token={accessToken}&v=5.80")]
        Task<VkNewsFeedGetResponse> GetFeedFirstPage(string accessToken);

        [Get("/method/newsfeed.get?return_banned=0&count=100&filters=post&start_from={startFrom}&access_token={accessToken}&v=5.80")]
